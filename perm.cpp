...
void perm::gen(unsigned int &d1, unsigned int &d2, unsigned int &d3, unsigned int &d4)
{
	//store things
	d1 = data.pass_i[0];
	d2 = data.pass_i[1];
...
void perm::gen(unsigned int * data, int num)
{
	int pos = 0;
	for(int i=0;i<num;i++)
		perm::gen(data[i*4+0],data[i*4+1],data[i*4+2],data[i*4+3]);//Up to 15 symbols length
